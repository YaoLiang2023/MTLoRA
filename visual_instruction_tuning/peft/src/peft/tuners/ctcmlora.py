...
@dataclass
class CTCMLoraConfig(LoraConfig):
    """
    ...
    def __post_init__(self):
        self.peft_type = PeftType.CTCMLORA


class CTCMLoraModel(LoraModel):
    """
    Creates CTCMLoRA model from a pretrained transformers model. Paper:
    https://arxiv.org/pdf/2501.08008

    Args:
        model ([`transformers.PreTrainedModel`]): The model to be adapted.
        config ([`CTCMLoraConfig`]): The configuration of the AdaLora model.
    ...
    Example::
        ...
        >>> from transformers import AutoModelForSeq2SeqLM, LoraConfig >>> from peft import CTCMLoraModel, CTCMLoraConfig
        >>> config = CTCMLoraConfig(
                peft_type="CTCMLORA", task_type="SEQ_2_SEQ_LM", r=8, lora_alpha=32, target_modules=["q", "v"],
                lora_dropout=0.01,
            )
        >>> model = AutoModelForSeq2SeqLM.from_pretrained("t5-base") >>> model = CTCMLoraModel(config, model)

    **Attributes**:
        - **model** ([`transformers.PreTrainedModel`]) -- The model to be adapted.
        - **peft_config** ([`CTCMLoraConfig`]): The configuration of the AdaLora model.
    """
    ...
    def add_adapter(self, adapter_name, config=None):
        if config is not None:
            ...
            config = self._prepare_ctcmlora_config(config, model_config)
            self.peft_config[adapter_name] = config
        self._find_and_replace(adapter_name)
        if len(self.peft_config) > 1 and self.peft_config[adapter_name].bias != "none":
            raise ValueError(
                "CTCMLoraModel supports only 1 adapter with bias. When using multiple adapters, set bias to 'none' for all adapters."
            )
        ...
        if traininable_mode_counter > 1:
            raise ValueError(
                "CTCMLoraModel supports only 1 trainable adapter. "
            ...
            if target_module_found:
                ...
                if isinstance(target, LoraLayer):
                    ...
                    if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                        ...
                        new_module = CTCMLinear(adapter_name, in_features, out_features, bias=bias, **kwargs)
    ...
    def _unload_and_optionally_merge(self, merge=True):
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer):
                ...
                if merge:
                    print(f"start merging CTCMLoRA key: {key}")
                    target.merge()
                self._replace_module(parent, target_name, new_module, target)
    ...
    @staticmethod
    def _prepare_ctcmlora_config(peft_config, model_config):
...
class CTCMLoraLayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            ...
            print("=====================================Enter the new CTCMLoraLayer linear method========================================")
...
class CTCMLinear(nn.Linear, CTCMLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        CTCMLoraLayer.__init__(self, in_features=in_features, out_features=out_features)
    ...
    def unmerge(self):
        print("start CTCMLoRA unmerging")
        if self.active_adapter not in self.lora_A.keys():
            return
        if not self.merged:
...
if is_bnb_available():
    ...
    class CTCMLinear8bitLt(bnb.nn.Linear8bitLt, CTCMLoraLayer):
...
if is_bnb_4bit_available():
    ...
    class CTCMLinear4bit(bnb.nn.Linear4bit, CTCMLoraLayer):
        # Low-rank matrix for SVD-based adaptation
        def __init__(
            self,
            adapter_name,
